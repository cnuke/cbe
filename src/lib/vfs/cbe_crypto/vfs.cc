...
class Vfs_cbe_crypto::Encrypt_file_system : public Vfs::Single_file_system
{
	private:

		Cbe_crypto::Interface &_crypto;
		uint32_t  _key_id;
	...
	public:
		...
		Encrypt_file_system(Cbe_crypto::Interface &crypto, uint32_t key_id)
		:
...
class Vfs_cbe_crypto::Decrypt_file_system : public Vfs::Single_file_system
{
	private:
	...
	public:
		...
		Decrypt_file_system(Cbe_crypto::Interface &crypto, uint32_t key_id)
		:
			Single_file_system { Node_type::TRANSACTIONAL_FILE, type_name(),
...
struct Vfs_cbe_crypto::Key_local_factory : File_system_factory
{
	Encrypt_file_system _encrypt_fs;
	Decrypt_file_system _decrypt_fs;

	Key_local_factory(Cbe_crypto::Interface &crypto,
	                  uint32_t               key_id)
	:
...
class Vfs_cbe_crypto::Key_file_system : private Key_local_factory,
...
class Vfs_cbe_crypto::Keys_file_system : public Vfs::File_system
{
	private:
		...
		struct Key_registry
		{
			...
			Key_registry(Genode::Allocator &alloc, Cbe_crypto::Interface &crypto)
			:
				_alloc  { alloc },
				_crypto { crypto }
			{ }
			...
			void update(Vfs::Env &vfs_env)
			{
				_crypto.for_each_key([&] (uint32_t const id) {
					...
					if (!already_known) {
						DBG(__func__, ":", __LINE__, " new id: ", id);
				...
				auto find_stale_keys = [&] (Key_file_system const &fs) {
					...
					if (!active_key) {
						DBG(__func__, ":", __LINE__, " stale id: ", fs.key_id());
...
class Vfs_cbe_crypto::Management_file_system : public Vfs::Single_file_system
{
	public:
	...
	public:
		...
		Management_file_system(Cbe_crypto::Interface &crypto, Type type, char const *type_name)
		:
			Single_file_system { Node_type::TRANSACTIONAL_FILE, type_name,
...
struct Vfs_cbe_crypto::Add_key_file_system : public Vfs_cbe_crypto::Management_file_system
{
	static char const *type_name() { return "add_key"; }

	Add_key_file_system(Cbe_crypto::Interface &crypto)
...
struct Vfs_cbe_crypto::Remove_key_file_system : public Vfs_cbe_crypto::Management_file_system
{
	static char const *type_name() { return "remove_key"; }

	Remove_key_file_system(Cbe_crypto::Interface &crypto)
...
struct Vfs_cbe_crypto::Local_factory : File_system_factory
{
...
class Vfs_cbe_crypto::File_system : private Local_factory,
                                    public Vfs::Dir_file_system
{
	private:
	...
	public:
		...
		:
			Local_factory        { vfs_env, Cbe_crypto::get_interface() },
		                         ...
		                         Genode::Xml_node node) override
		{
			try {
				return new (vfs_env.alloc())
					Vfs_cbe_crypto::File_system(vfs_env, node);
			} catch (...) {
				Genode::error("could not create 'cbe_crypto_aes_cbc'");
			}
			return nullptr;
		}
	};
